...
8. **正解発表 → 次へ** — イントロモードの正解発表中は曲をフル再生（ループ）。ジャケットモードでは元のジャケットとアルバム名を表示し、MusicKitでそのアルバム全体を曲順どおりにリピート再生します。次のラウンドや結果発表で停止します。ホストは「次のラウンドへ」で次の曲、「結果発表へ」でスコアボードを表示します。
