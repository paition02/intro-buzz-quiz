...
5. **ヒント提示** — イントロモードでは再生秒数（0.1〜30 秒）を指定して「再生」。ジャケットモードでは難読化モード、グレースケール、ヒントレベル（1〜100%）を調整します。
...
8. **正解発表 → 次へ** — イントロモードの正解発表中は曲をフル再生（ループ）。ジャケットモードでは元のジャケットとアルバム名を表示します。ホストは「次のラウンドへ」で次の曲、「結果発表へ」でスコアボードを表示します。
...
- **難読化モード** — 粗解像度、ブロック欠落、タイルシャッフル、円形スポット開示、拡大回転クロップ、外周開示
- **グレースケール** — 各難読化モードに重ねられる ON/OFF 設定
...
| `console:set-jacket-mode` | ジャケット難読化モードを変更 |
| `console:set-jacket-grayscale` | ジャケットのグレースケール ON/OFF を変更 |
