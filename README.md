...
- **Bun ネイティブ** — ランタイム・HTTP サーバー・WebSocket をすべて Bun（`Bun.serve` + `@socket.io/bun-engine`）で動かします。ビルドステップなしで `index.html` を直接配信します。
...
`bun --hot` でホットリロード付きで起動します。既定ポートは `5173`、`0.0.0.0` で待ち受けるため LAN 内の端末からアクセスできます。

| 用途 | URL |
| --- | --- |
| ホスト操作（PC / スマホ） | <http://localhost:5173/console> |
| 大画面表示 | <http://localhost:5173/gameboard> |
| 早押しボタン（各プレイヤーのスマホ） | <http://localhost:5173/action> |

LAN 内の別端末から使う場合は `localhost` を PC の IP アドレスに置き換えます（例: `http://192.168.x.x:5173/action`）。物理ボタンは `POST http://<PCのIP>:5173/api/act/<任意のID>` を叩きます。
